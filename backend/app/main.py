...
async def register_user(
    user_data: schemas.UserCreate, db: AsyncSession = Depends(get_session)
):
...
@app.get("/polls", response_model=List[schemas.PollSummary])
async def get_polls(
    db: AsyncSession = Depends(get_session),
    ...
    result = await db.execute(
        ...
        # Optimized: Exclude expensive join on questions/options for the list view
...
async def get_poll(
    ...
    poll_result = await db.execute(
        select(models.Poll)
        .where(models.Poll.id == poll_id, models.Poll.instructor_id == current_user.id)
        .options(
    ...
    if not poll:
        raise HTTPException(
            status_code=404, detail="Poll not found or not owned by user"
        )
    return poll


async def _create_questions_for_poll(
    questions_data: List[schemas.QuestionCreate], poll_id: int, db: AsyncSession
):
        ...
        if q_data.options:
            for option_text in q_data.options:
                new_option = models.Option(
                    question_id=new_question.id, text=option_text
                )
                db.add(new_option)
...
async def delete_poll(
    ...
    poll_result = await db.execute(
        select(models.Poll).where(
            models.Poll.id == poll_id, models.Poll.instructor_id == current_user.id
        )
    )
    poll = poll_result.scalars().first()
...
async def _update_question_options(
    question: models.Question, options_data: Optional[List[str]], db: AsyncSession
):
    """Helper to delete all existing options for a question and create new ones."""
    await db.execute(
        models.Option.__table__.delete().where(models.Option.question_id == question.id)
    )
...
async def update_question(
    ...
    question_result = await db.execute(
        select(models.Question).where(
            models.Question.id == question_id, models.Question.poll_id == poll_id
        )
    )
...
async def _get_tally_for_question(question: models.Question, db: AsyncSession) -> dict:
    ...
    if question.type in (
        ...
        responses_query = select(models.Vote.response_value).where(
            models.Vote.question_id == question.id
        )
...
async def get_poll_results(
    ...
    return {"poll_id": poll_full.id, "results": results}


@app.get("/polls/access/{access_code}", response_model=schemas.PollPublic)
async def get_poll_by_access_code(
    access_code: str, db: AsyncSession = Depends(get_session)
):
    result = await db.execute(
        select(models.Poll)
...
async def submit_vote(
    access_code: str, vote: schemas.VoteCreate, db: AsyncSession = Depends(get_session)
):
    result = await db.execute(
        select(models.Poll).where(models.Poll.access_code == access_code)
    )
    poll = result.scalars().first()
    ...
    if question.type == models.QuestionType.OPEN_ENDED and question.word_limit:
        ...
        if len(words) > question.word_limit:
            raise HTTPException(
                status_code=400,
                detail=f"Word limit exceeded. Maximum {question.word_limit} words allowed.",
            )
    ...
    if tally_data:
        await manager.broadcast(
            str(poll.id), json.dumps({"type": "tally_update", "data": tally_data})
        )
    ...
    await manager.broadcast(
        str(poll.id),
        json.dumps(
            {"type": "participant_update", "data": {"count": participant_count}}
        ),
    )
...
async def submit_batch_votes(
    access_code: str,
    batch_vote: schemas.BatchVote,
    db: AsyncSession = Depends(get_session),
):
    result = await db.execute(
        select(models.Poll).where(models.Poll.access_code == access_code)
    ...
    for q_id in affected_question_ids:
        ...
        if question:
            ...
            if tally_data:
                await manager.broadcast(
                    str(poll.id),
                    json.dumps({"type": "tally_update", "data": tally_data}),
                )
...
def _build_initial_state_message(poll: models.Poll, initial_tally: List[dict]) -> str:
    ...
    message = {
        ...
        "data": {
            ...
            "participant_count": 0,
        },  # Default 0, overridden by passed arg if needed? Better to pass it.
    }
    return json.dumps(message)


def _build_initial_state_message_with_count(
    poll: models.Poll, initial_tally: List[dict], participant_count: int
) -> str:
    poll_dict = {
        "id": poll.id,
        "title": poll.title,
...
async def websocket_endpoint(
    websocket: WebSocket, poll_id: str, db: AsyncSession = Depends(get_session)
):
    ...
    try:
        ...
        initial_message = _build_initial_state_message_with_count(
            poll, initial_tally, participant_count
        )
